...
class QuizSignalrDataSourceImpl implements QuizSignalrDataSource {
  ...
  Future<void> connect(String url, String token) async {
    print('🌐 QUIZZES SIGNALR: Attempting to connect to $url');
    ...
    try {
      ...
      print('✅ QUIZZES SIGNALR: Connected Successfully!');
    } catch (e) {
      print('❌ QUIZZES SIGNALR: Connection Failed! Error: $e');
    }
  }
  ...
  void onNewQuizReceived(Function(dynamic) callback) {
    _hubConnection?.on('ReceiveNewQuiz', (arguments) {
      if (arguments != null && arguments.isNotEmpty) {
        try {
          print('🔥 QUIZZES SIGNALR: Received New Quiz -> ${arguments.first}');
          callback(arguments.first);
        } catch(e) { 
          print('❌ QUIZZES PARSE ERROR: $e'); 
        }
      }
    });
  }
